...
import { ThumbsUp, MessageSquare, Share2, MoreHorizontal, Globe, Check, Send, Sparkles, Image as ImageIcon, Tv, Mic } from "lucide-react";
  ...
  return (
    <div className="w-full max-w-full bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden mb-3.5 sm:mb-4 transition-all min-w-0 box-border" id={`post-${post.id}`}>
      ...
      <div className="px-3 sm:px-4 pb-3 min-w-0">
        {/* Anime Title in description bolded */}
        {post.title && (
          <h3 
      ...
      <div className="flex items-center justify-between px-1 sm:px-2 py-1 text-gray-600 font-bold text-xs sm:text-sm select-none min-w-0">
        <button
          ...
          className={`flex items-center justify-center gap-1.5 sm:gap-2 flex-1 py-1.5 sm:py-2 rounded-xl transition-all duration-150 cursor-pointer min-w-0 ${post.isLikedByUser ? 'text-[#1877F2] bg-blue-50/50 scale-102' : 'hover:bg-gray-100'}`}
        >
          <ThumbsUp className={`h-4 w-4 sm:h-4.5 sm:w-4.5 shrink-0 ${post.isLikedByUser ? 'fill-[#1877F2]' : ''}`} />
